...
class trainerdata:
    def __init__(self):
        game = ''
        trainer_name_list_royale = ['trainer_list_royale.csv']
        trainer_name_list_tree = ['trainer_list_tree.csv']
        trainer_name_list_world = ['trainer_list_world.csv']
        ability_name_list = ['custom_ability_list.csv']
        item_name_list = ['custom_item_list.csv']
        pokemon_name_list = ['custom_pokemon_list.csv']
        move_name_list = ['custom_move_list.csv']
...
def print_facility_trainer_csv(working_data):
    ...
    with open(asksaveasfilename(title='Save Table of Facility Pokemon', defaultextension='.csv',filetypes= [('CSV','.csv')]), 'w', newline = '', encoding='utf-8-sig') as csvfile:
        ...
        for count, entry in enumerate(working_data.pokemon_binary):
            ...
            #pokemon index:
            pokemon_index = working_data.pokemon_name_list[from_little_bytes_int(entry[0:2])]
            ...
            writer_head.writerow([
                count, 
                working_data.pokemon_name_list[pokemon_index], 
                '' if forme_number == 0 else working_data.pokemon_name_list[forme_number], 
                *(working_data.move_name_list[from_little_bytes_int(entry[2 + x: 4 + x])] for x in range(4)), 
                ivs, 
    ...
    with open(asksaveasfilename(title='Save Table of Facility Trainers', defaultextension='.csv',filetypes= [('CSV','.csv')]), 'w', newline = '', encoding='utf-8-sig') as csvfile:
        ...
        for count, entry in enumerate(working_data.trainer_binary):
            ...
            #append trainer name
            trainer_output.append(tree_names[count])
            #append trainer class
            trainer_output.append[class_names[from_little_bytes_int(entry[0x0:0x2])]]
            #append mystery value
            trainer_output.append[class_names[from_little_bytes_int(entry[0x2:0x4])]]
...
def export_from_GARC(working_data, target_choice):

    #each entry in the array is a file in the deconstructed GARC
    working_data.trainer_binary = load_GARC(working_data.game, askopenfilename(title=f'Select Trainer Data {working_data.trainer_file_name}', defaultextension='',filetypes= [('','')]), target_choice)

    working_data.pokemon_binary = load_GARC(working_data.game, askopenfilename(title=f'Select Pokemon Data {working_data.pokemon_file_name}', defaultextension='',filetypes= [('','')]), target_choice)

    working_data.personal_binary = load_GARC(working_data.game, askopenfilename(title=f'Select Personal Data {working_data.pokemon_file_name}', defaultextension='',filetypes= [('','')]), 'Personal')

    #build table per trainer
    if(target_choice == 'Regular Trainers'):
    ...
    #built two seperate files, one with trainers and what Pokemon indices and the other with the Pokemon
        ...
        print_facility_trainer_csv(working_data)
...
def main():
    ...
    with open(os.path.join(os.getcwd(), 'paths.cfg'), 'r') as cfg:
        ...
        load_text_from_csv(os.path.join(temp[0], working_data.trainer_name_list_royale[0]), working_data.trainer_name_list_royale)
        load_text_from_csv(os.path.join(temp[0], working_data.trainer_name_list_tree[0]), working_data.trainer_name_list_tree)
        load_text_from_csv(os.path.join(temp[0], working_data.trainer_name_list_world[0]), working_data.trainer_name_list_world)
        load_text_from_csv(os.path.join(temp[0], working_data.ability_name_list[0]), working_data.ability_name_list)
        load_text_from_csv(os.path.join(temp[0], working_data.item_name_list[0]), working_data.item_name_list)
        load_text_from_csv(os.path.join(temp[0], working_data.pokemon_name_list[0]), working_data.pokemon_name_list)
        load_text_from_csv(os.path.join(temp[0], working_data.move_name_list[0]), working_data.move_name_list)
